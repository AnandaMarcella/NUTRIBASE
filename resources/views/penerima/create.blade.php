...
@section('title', 'Lengkapi Profil Penerima — NutriBase')
    ...
    <main class="flex-1 p-4 sm:p-6 lg:p-8 lg:pl-64">
        <div class="max-w-3xl mx-auto">
            <div class="bg-white rounded-3xl shadow-sm border border-[#DFF0E5] p-6 mb-6">
                <h1 class="text-2xl font-bold text-[#4E6F5C]">Lengkapi Profil Penerima</h1>
                <p class="text-sm text-[#6B7A6F] mt-2">Isi data penerima agar Anda bisa melihat status distribusi dan riwayat bantuan.</p>
            </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-[#DFF0E5] p-6">
                <form method="POST" action="{{ route('penerima.store') }}">
                    @csrf
                    @include('penerima.form_page', ['mode' => 'create'])
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row gap-3">
                        <button type="submit" class="inline-flex items-center justify-center px-5 py-3 bg-[#06B13D] hover:bg-[#059933] text-white text-sm font-semibold rounded-2xl transition">
                            <i class="bi bi-check-lg mr-2"></i> Simpan Profil
                        </button>
