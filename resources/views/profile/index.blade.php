@extends('layouts.app')
@section('title', 'Profil Saya — NutriBase')
@section('content')
            ...
            <div class="bg-white rounded-3xl shadow p-6 mb-4 border border-gray-100">
                ...
                <form action="{{ route('profile.update') }}" method="POST"
                    ...
                    <div class="flex items-center gap-3 my-6">
                        ...
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest">Ganti Username</p>
                        <div class="flex-1 h-px bg-gray-200"></div>
                    </div>
...
<div class="mb-4">
    ...
    <input
        ...
        class="w-full bg-gray-100 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#79C80E]/50"
        :class="newPassword.length > 0 && newPassword.length < 6 ? 'ring-2 ring-red-400' : ''">

    {{-- Pesan Validasi Panjang Karakter --}}
    <div x-show="newPassword.length > 0 && newPassword.length < 6" class="mt-1">
        <p class="text-red-500 text-[10px] font-medium">
    ...
    <div x-show="newPassword.length > 0" class="mt-2">
        <div class="flex items-center justify-between mb-1">
            <span class="text-[10px] font-bold uppercase text-gray-400" x-text="'Kekuatan: ' + strengthLabel"></span>
        </div>
...
<div class="mb-6" x-data="{ confirmPassword: '' }">
    <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400 block mb-1">
        Konfirmasi Password Baru
    </label>
    ...
    <input
        ...
        class="w-full bg-gray-100 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#79C80E]/50"
        :class="confirmPassword.length > 0 && confirmPassword !== newPassword ? 'ring-2 ring-red-400' : ''">

    {{-- Pesan Validasi Mismatch --}}
    <div x-show="confirmPassword.length > 0 && confirmPassword !== newPassword" class="mt-1">
        <p class="text-red-500 text-[10px] font-medium">
            <i class="bi bi-x-circle"></i> Password konfirmasi tidak cocok!
        </p>
    </div>
    
    <div x-show="confirmPassword.length > 0 && confirmPassword === newPassword" class="mt-1">
        <p class="text-[#06B13D] text-[10px] font-medium">
