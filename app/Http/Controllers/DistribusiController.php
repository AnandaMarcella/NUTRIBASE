...
class DistribusiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // ✅ BUG 1 FIX: Kurangi stok menu jika status diterima
    ...
    public function update(Request $request, Distribusi $distribusi)
    {
        ...
        // ✅ BUG 1 FIX: Sesuaikan stok jika status berubah
        $statusLama   = $distribusi->status;
        $statusBaru   = $validated['status'];
        $menuIdLama   = $distribusi->menu_id;
        $menuIdBaru   = $validated['menu_id'];

        // Kembalikan stok menu lama jika status sebelumnya diterima
        if ($statusLama === 'diterima') {
            $menuLama = Menu::find($menuIdLama);
            if ($menuLama) {
                $menuLama->increment('stok');
            }
        }

        // Kurangi stok menu baru jika status baru diterima
        if ($statusBaru === 'diterima') {
            $menuBaru = Menu::findOrFail($menuIdBaru);
            if ($menuBaru->stok > 0) {
                $menuBaru->decrement('stok');
            }
        }
    ...
    public function destroy(Distribusi $distribusi)
    {
        ...
        // ✅ BUG 1 FIX: Kembalikan stok jika distribusi yang dihapus berstatus diterima
        if ($distribusi->status === 'diterima') {
            $menu = Menu::find($distribusi->menu_id);
            if ($menu) {
                $menu->increment('stok');
            }
        }
